...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьФорматированиеСтроки");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьФорматированиеЧегоПопало");
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьФорматированиеСмещениеРазрядов");
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьФорматированиеЛокальнаяДата_НеверныйФормат");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьФорматирование_ПриоритетФорматов");
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьБезформатныйФорматДат");
...
Процедура ТестДолжен_ПроверитьФорматированиеБулево() Экспорт
	...
	юТест.ПроверитьРавенство(Строка(Да), Формат(Да, "БИ='неверно заданная строка"));
	юТест.ПроверитьРавенство("123", Формат(Да, "БИ=  123;;БИ='неверно заданная строка"));
	юТест.ПроверитьРавенство("", Формат(Да, "БИ=;;БИ='неверно заданная строка"));

	юТест.ПроверитьРавенство(Строка(Да), Формат(Да, "БИ БИ='неверно заданная строка';"));
	юТест.ПроверитьРавенство(Строка(Да), Формат(Да, "Б И='неверно заданное имя';"));
	юТест.ПроверитьРавенство(Строка(Да), Формат(Да, "Б,БИ='неверно заданное имя';"));
	юТест.ПроверитьРавенство(Строка(Да), Формат(Да, ",БИ='неверно заданное имя';"));
	юТест.ПроверитьРавенство(Строка(Да), Формат(Да, ";;,БИ='неверно заданное имя';"));
	юТест.ПроверитьРавенство("123", Формат(Да, ";;БИ='123';"));
...
Процедура ТестДолжен_ПроверитьФорматированиеРазмерностиЧисло() Экспорт
	...
	юТест.ПроверитьРавенство("0.0", Формат(0.01, "ЧДЦ=1; L=en_US"));
...
Процедура ТестДолжен_ПроверитьФорматированиеСмещениеРазрядов() Экспорт

	Число = 123456.789;
	юТест.ПроверитьРавенство("123456.789", Формат(Число, "ЧГ=0; ЧС=; L=""en_US"""));
	юТест.ПроверитьРавенство("1234567.89", Формат(Число, "ЧГ=0; ЧС=-1; L=""en_US"""));
	юТест.ПроверитьРавенство("12345.6789", Формат(Число, "ЧГ=0; ЧС= 1  ; L=""en_US"""));
...
Процедура ТестДолжен_ПроверитьФорматированиеЛокальнаяДата() Экспорт
	...
	юТест.ПроверитьРавенство("2/7/2014", Формат(Эталон, "ДЛФ=Д; Л=en_US"));
	юТест.ПроверитьРавенство("12/17/2014", Формат(Эталон2, "ДЛФ=Д; Л=en_US"));

КонецПроцедуры

Функция НаборТестовыхДат()

	МассивПроверяемыхДат = Новый Массив;
	МассивПроверяемыхДат.Добавить('00010101');
	МассивПроверяемыхДат.Добавить('20010101');
	МассивПроверяемыхДат.Добавить('20211217');
	МассивПроверяемыхДат.Добавить('20211217123456');

	Возврат МассивПроверяемыхДат;

КонецФункции

Процедура ТестДолжен_ПроверитьФорматированиеЛокальнаяДата_НеверныйФормат() Экспорт

	Для Каждого Эталон Из НаборТестовыхДат() Цикл

		юТест.ПроверитьРавенство(
			Формат(Эталон, "ДЛФ=ЗаведомоПлохойФормат"),
			Формат(Эталон, ""),
			"Дата с неверным форматом равна формату по-умолчанию");

	КонецЦикла;

КонецПроцедуры

Процедура ТестДолжен_ПроверитьФорматирование_ПриоритетФорматов() Экспорт

	Для Каждого Эталон Из НаборТестовыхДат() Цикл

		юТест.ПроверитьРавенство(
			Формат(Эталон, "ДЛФ=ДВ; ДФ=гггг!ММ!дд!ЧЧ!мм!сс"),
			Формат(Эталон, "ДФ=гггг!ММ!дд!ЧЧ!мм!сс"),
			"ДФ имеет приоритет над ДЛФ");
...
Процедура ТестДолжен_ПроверитьПроизвольноеФорматированиеДат() Экспорт
	...
	юТест.ПроверитьРавенство("AM", Формат('20211019', "ДФ=вв"));
	юТест.ПроверитьРавенство("AM", Формат('20211019110000', "ДФ=вв"));
	юТест.ПроверитьРавенство("PM", Формат('20211019120000', "ДФ=вв"));
	юТест.ПроверитьРавенство("PM", Формат('20211019150000', "ДФ=вв"));
	юТест.ПроверитьРавенство("PM", Формат('20211019235959', "ДФ=вв"));

	юТест.ПроверитьРавенство("AM", Формат('20211019', "ДФ=tt"));
	юТест.ПроверитьРавенство("AM", Формат('20211019110000', "ДФ=tt"));
	юТест.ПроверитьРавенство("PM", Формат('20211019120000', "ДФ=tt"));
	юТест.ПроверитьРавенство("PM", Формат('20211019150000', "ДФ=tt"));
	юТест.ПроверитьРавенство("PM", Формат('20211019235959', "ДФ=tt"));
...
Процедура ТестДолжен_ПроверитьБезформатныйФорматДат() Экспорт

	юТест.ПроверитьРавенство(Формат('00010101', ""), "");
	юТест.ПроверитьРавенство(Формат('20211016', ""), Строка('20211016'));

КонецПроцедуры

Процедура ТестДолжен_ПроверитьФорматированиеСтроки() Экспорт
	юТест.ПроверитьРавенство(Формат("123", ""), "123", "Формат строки - строка");
КонецПроцедуры

Процедура ТестДолжен_ПроверитьФорматированиеЧегоПопало() Экспорт

	Попытка
		Значене = Формат(Новый Массив, "");
		ВызваноИсключение = Ложь;
	Исключение
		ВызваноИсключение = Истина;
	КонецПопытки;

	юТест.ПроверитьИстину(ВызваноИсключение, "Форматирование Массива должно вызвать исключение, а не вызвало!");
