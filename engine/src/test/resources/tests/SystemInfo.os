...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	// Тесты отключены до лучших времен
...
Процедура ТестДолжен_ПолучитьВерсиюOneScript() Экспорт
	...
	ЧастиВерсии = СтрРазделить(Си.Версия, ".");
	юТест.ПроверитьРавенство(ЧастиВерсии.Количество(), 3); // x.y.z
...
Процедура ТестДолжен_ПолучитьТипПлатформы() Экспорт
	...
	Если Найти(СИ.ВерсияОС, "Windows") > 0 Тогда
		...
		КонецЕсли;
	ИначеЕсли Найти(СИ.ВерсияОС, "Mac OS X") > 0 Тогда
		Если Си.Это64БитнаяОперационнаяСистема Тогда
			юТест.ПроверитьРавенство(СИ.ТипПлатформы, ТипПлатформы.MacOS_x86_64);
		Иначе
			юТест.ПроверитьРавенство(СИ.ТипПлатформы, ТипПлатформы.MacOS_x86);
		КонецЕсли;
	Иначе
		Если Си.Это64БитнаяОперационнаяСистема Тогда
