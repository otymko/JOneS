...
Процедура ТестДолжен_СоздатьФиксированноеСоответствиеПоСоответствию() Экспорт
	...
	Ожидаем.Что(ФиксированноеСоответствие.Количество(), "количество элементов исходной и полученной Соответствия совпадут")
		.Равно(Соответствие.Количество());

	ЕстьОшибки = Ложь;
	Попытка
		ФиксированноеСоответствие = Новый ФиксированноеСоответствие(Неопределено);
	Исключение
		ЕстьОшибки = Истина;
	КонецПопытки;

	Ожидаем.Что(ЕстьОшибки, "Наличие ошибки в некорреткном конструкторе").Равно(Истина);
...
Процедура ТестДолжен_ПроверитьЧтоСуществующийКлючНельзяИзменить() Экспорт
	...
	Исключение
		...
		Если Не ИнформацияОбОшибке().Описание = "Свойство Ключ1 недоступно для записи" Тогда
			ВызватьИсключение;
		КонецЕсли;
	КонецПопытки;
