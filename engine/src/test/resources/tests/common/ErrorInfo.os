...
Процедура ТестДолжен_ПроверитьЛокальнуюИнформациюОбОшибке() Экспорт
    ...
    Исключение
        ...
        ЮТесты.ПроверитьРавенство("Деление на ноль", Информация.Описание);
        ...
        ЮТесты.ПроверитьИстину(СтрЗаканчиваетсяНа(Информация.ИсходнаяСтрока, "Сумма = 100 / 0;"));
        ЮТесты.ПроверитьИстину(Информация.Причина = Неопределено);

        ПодробноеОписаниеОшибки = Информация.ПодробноеОписаниеОшибки();
        ЮТесты.ПроверитьИстину(СтрНайти(ПодробноеОписаниеОшибки, "Деление на ноль") > 0);
    КонецПопытки;
КонецПроцедуры
