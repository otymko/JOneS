...
Процедура ТестДолжен_ОтработатьСДанными() Экспорт
	...
	Т.Очистить();
	юТест.ПроверитьРавенство(Т.Итог("Сумма"), 0, "Итог по пустой таблице = 0");

	Т.Колонки.Добавить("КолонкаСНеопределено");
	НоваяСтрока = Т.Добавить();
	юТест.ПроверитьРавенство(Т.Итог("КолонкаСНеопределено"), 0, "Итог смешанному типу без полезных данных");

	НоваяСтрока = Т.Добавить();
	НоваяСтрока.КолонкаСНеопределено = 5;
	юТест.ПроверитьРавенство(Т.Итог("КолонкаСНеопределено"), 5, "Итог смешанному типу с полезными данными");

	НоваяСтрока = Т.Добавить();
	НоваяСтрока.КолонкаСНеопределено = "5";
	юТест.ПроверитьРавенство(Т.Итог("КолонкаСНеопределено"), 5, "Данные не должны приводиться к числу");
