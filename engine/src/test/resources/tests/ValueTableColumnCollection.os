...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьОшибкиСоздания");
...
Процедура ТестДолжен_СоздатьКолонкуТаблицыЗначений() Экспорт
	...
	юТест.ПроверитьРавенство(Т.Колонки[0], К0);
	юТест.ПроверитьРавенство(Т.Колонки[1], К1);
...
Процедура ТестДолжен_ПроверитьОшибкиСоздания() Экспорт
	...
	К1 = Т.Колонки.Добавить("Колонка1");
	К2 = Т.Колонки.Добавить("Колонка2");
	КУ = Т.Колонки.Добавить("КолонкаУдалена");
	Т.Колонки.Удалить(КУ);
	...
	Исключение
		БылоИсключение = Истина;
	КонецПопытки;
	юТест.ПроверитьИстину(БылоИсключение, "Добавление с существующим именем");

	БылоИсключение = Ложь;
	Попытка
		К0 = Т.Колонки.Добавить("");
	Исключение
		БылоИсключение = Истина;
	КонецПопытки;
	юТест.ПроверитьИстину(БылоИсключение, "Добавление с пустым именем");

	БылоИсключение = Ложь;
	Попытка
		К0 = Т.Колонки.Добавить(Неопределено);
	Исключение
		БылоИсключение = Истина;
	КонецПопытки;
	юТест.ПроверитьИстину(БылоИсключение, "Добавление с пустым именем");
	...
	БылоИсключение = Ложь;
	Попытка
		Т.Колонки.Сдвинуть(КУ, 1);
	Исключение
		БылоИсключение = Истина;
	КонецПопытки;
	юТест.ПроверитьИстину(БылоИсключение, "Сдвиг несуществующей колонки");

	БылоИсключение = Ложь;
	Попытка
		Т.Колонки.Сдвинуть(-1, 1);
	Исключение
		БылоИсключение = Истина;
	КонецПопытки;
	юТест.ПроверитьИстину(БылоИсключение, "Сдвиг несуществующей колонки");
	...
	Исключение
		БылоИсключение = Истина;
	КонецПопытки;
	юТест.ПроверитьИстину(БылоИсключение, "Установка по индексу");
